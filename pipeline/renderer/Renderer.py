...
class Renderer:
    ...
    def _worker_loop(self):
        ...
        while True:
            ...
            try:
                ...
                print(f"Rendering {task_type} success")
                self.result_queue.put((True, res))
            except Exception as e:
                import traceback
                print(f"Rendering {task_type} error")
                traceback.print_exc()
    ...
    def render_dlss_comparison(self, camera_info: dict):
        print(" render_dlss_comparison to task_queue...")
        self.task_queue.put(('render_dlss_comparison', (camera_info,), {}))
        success, result = self.result_queue.get()
        if not success:
            print("render_dlss_comparison failed")
            raise result
    ...
    def render_with_controlnet(self, b64_depth: str, prompt: str, camera_info: dict):
        ...
        # ==========================================
        # 3. Upscale image input for ControlNet
        # ==========================================
    ...
    def _render_benchmark(self, camera_info: dict):
        ...
        Returns:
            save a grid image called benchmark.png and psnr/ssim score string as a txt or other
            
            low-res(fbo)       / high-res
            bilinear      / upscale_model_output
            ground_truth(remember to resize same as upscale_model_output and bilinear)
